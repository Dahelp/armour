<?=\app\models\Breadcrumbs::render([
	...
	['label' => \ishop\App::upFirstLetter($type['seoname_3']) . ' ' . $manufacturer['name'], 'url' => PATH . '/technics/' . $type['alias'] . '/' . $manufacturer['alias']],
	['label' => $type->name . ' ' . $manufacturer->name . ' ' . $technics->model],
])?>
<?php
    ...
    <div class="container">
        <section class="ps-lg-4 pe-lg-3">
		  ...
		  <div class="catalog_text">
			...
			<p><?php if($type->name != "Квадроцикл") { ?>Спецтехника<?php }else{ ?>Квадроцикл<?php } ?>, которая интенсивно эксплуатируется с большими нагрузками, требует регулярной замены резины. При этом шины для <?=$type["seoname_1"]?> должны быть качественными, прочными и износостойкими. Всем этим критериям отвечает резина различных производителей, представленная в интернет-магазине «ТехШина».</p>
			<p>На данный тип <?php if($type->name != "Квадроцикл") { ?>спецтехники<?php }else{ ?>техники<?php } ?> <?=$type->name?> <?=$manufacturer->name?> <?=$technics->model?> мы предлагаем шины с размерами наружного и посадочного диаметра <?=$vsesize?>. Шины отличаются по материалу и способу изготовления и делятся на два типа: <?php if($type->name != "Квадроцикл") { ?>цельнолитые и диагональные<?php }else{ ?>направленный и ненаправленный рисунок протектора<?php } ?>.</p>
