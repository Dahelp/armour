			...
			<urlset
				...
				if($sm_atgroup){
					foreach($sm_atgroup as $smatg) {  
						...
						$sm_atvalue = \R::getAll("SELECT value FROM `attribute_value` WHERE attr_group_id = '".$smatg["id"]."' AND hide='show'");
						if($sm_atvalue){
							foreach($sm_atvalue as $atvl) {
								$text.= "<url><loc>".PATH."/".$smatg["url_params"]."/".$atvl['value']."</loc></url>";
							}
						}       
					}	 
				}
				...
				if($sm_cross){
					foreach($sm_cross as $cross) {     
						$text.= "<url><loc>".PATH."/cross/".$cross['cross_abbreviated_name']."</loc></url>";       
					}	 
				}
