        ...
        foreach($items as &$item){
            ...
            foreach($xpath->query('//img[@src]')?:[] as $image){
                ...
                if(!array_key_exists($src,$cache)){
                    $saved=(new RemoteImageDownloader())->download($src,$directory,10*1024*1024,(string)($item['source_url']??''));
                    $dimensions=@getimagesize($saved['path']);
                    if(!$dimensions||$dimensions[0]<100||$dimensions[1]<50){@unlink($saved['path']);$cache[$src]=null;}
                    else{$cache[$src]=rtrim($publicPrefix,'/').'/'.$saved['name'];$downloaded++;}
                }
                if($cache[$src]===null){$image->parentNode?->removeChild($image);continue;}
