...
class Brand extends AppModel {
	...
	public function uploadImg($name, $wmax, $hmax){
        $uploaddir = WWW . '/images/brand/baseimg/';
	...
	public function checkUnique(){
        $brand = \R::findOne('brand', 'name = ?', [$this->attributes['name']]);
        if($brand){
            if($brand->name == $this->attributes['name']){
                $this->errors['unique'][] = 'Название производителя уже существует';
            }
            return false;
        }
        return true;
