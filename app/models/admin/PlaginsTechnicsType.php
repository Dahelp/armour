...
class PlaginsTechnicsType extends AppModel {
	...
	public function uploadImg($name, $wmax, $hmax){
        $uploaddir = WWW . '/images/technics_type/baseimg/';
	...
	public function checkUnique(){
        $technics_types = \R::findOne('technics_type', 'name = ?', [$this->attributes['name']]);
        if($technics_types){
            if($technics_types->name == $this->attributes['name']){
                $this->errors['unique'][] = 'Название категории техники уже существует';
            }
            return false;
        }
        return true;
