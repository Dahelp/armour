...
if($apply){require dirname(__DIR__).'/config/init.php';}
printf("Проверено черновиков: %d.\n",count($rows));
if(!$apply){echo "Dry-run завершён: база данных не изменена. Для импорта добавьте --apply.\n";exit(0);}
\ishop\Db::instance();$count=(new \app\services\LegacyContentRepository())->insertDrafts($rows);echo "Импортировано скрытых черновиков: {$count}.\n";
